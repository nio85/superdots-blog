...
faqs:
  ...
  - question: "Do AI CRO tools actually work?"
    answer: "It depends on what the tool is actually doing. Tools that use AI to route visitors to better-performing variants — like Unbounce Smart Traffic — show measurable lift, typically 20% improvement in conversion rates according to Unbounce's published data. Tools that use AI only to generate insight reports are dashboards with a chatbot attached. They can surface patterns faster, but the optimization work still falls entirely on you."
