...
faqs:
  ...
  - question: "How much does an AI chatbot cost?"
    answer: "Free tiers exist for small volumes (Chatfuel, Tidio). Mid-range platforms run $50-200/month. Enterprise solutions like Intercom Fin and Zendesk AI charge per resolution ($0.99-2.00 per automated resolution) or per seat. ROI is typically positive within 1-2 months."
  - question: "How long does it take to deploy an AI chatbot?"
    answer: "Basic deployment takes 1-3 hours: connect your knowledge base, customize the widget, and go live. Full deployment with custom flows, integrations, and testing typically takes 1-2 weeks. Start simple and improve based on real conversations."
...
1. **Connect your knowledge base** to the chatbot platform
2. **Customize the chat widget** — match your brand colors, set the welcome message
3. **Define handoff rules** — when should the bot transfer to a human? (Common triggers: customer asks for a person, issue involves billing over $X, sentiment is negative)
4. **Set business hours** — what happens outside hours? (Queue for agents, or let the bot handle everything it can)

### Hour 3: Test and launch

1. **Test 20-30 common questions** — verify the bot answers correctly
2. **Test edge cases** — what happens with vague questions, off-topic requests, or aggressive language?
3. **Test handoff flow** — make sure the transition to a human agent preserves conversation context
4. **Launch in limited mode** — deploy to one channel or a percentage of traffic first

For a deeper guide to AI-powered support chatbots, see [AI customer service chatbot](/blog/ai-customer-service-chatbot/).
